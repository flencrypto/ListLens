...
function ActivityRow({ item }: { item: RecentActivity }) {
  ...
  return (
    <Link href={item.href}>
      <div className="group flex items-center gap-3 rounded-lg border border-[color:var(--brand-outline)] bg-[color:var(--brand-accent-soft)] p-3 transition hover:border-cyan-300/35 hover:bg-[color:var(--brand-accent-soft)]">
        <LensOrb icon={isStudio ? Camera : ShieldCheck} tone={isStudio ? "cyan" : "violet"} size="sm" />
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-[color:var(--brand-text-strong)] group-hover:text-cyan-200">{item.title}</p>
  ...
  return (
    <HudPanel tone={tone} className="p-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="font-mono-hud text-[10px] uppercase tracking-[0.18em] text-[color:var(--brand-text-muted)]">{label}</p>
          <p className="mt-2 text-3xl font-black text-[color:var(--brand-text-strong)]">{value}</p>
        </div>
...
function ListingRow({ listing }: { listing: ListingSummary }) {
  ...
  return (
    <Link href={listing.id.startsWith("demo") ? "/studio/new" : `/studio/${listing.id}`}>
      <div className="group grid gap-3 rounded-lg border border-[color:var(--brand-outline)] bg-[color:var(--brand-accent-soft)] p-4 transition hover:border-cyan-300/35 hover:bg-[color:var(--brand-accent-soft)] sm:grid-cols-[auto_1fr_auto] sm:items-center">
        <LensOrb icon={Icon} tone={listing.lens === "ShoeLens" ? "cyan" : "blue"} size="sm" />
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-[color:var(--brand-text-strong)]">{listing.title ?? `${listing.lens} draft`}</p>
          <p className="mt-1 text-xs text-[color:var(--brand-text-muted)]">
        ...
        <div className="flex items-center gap-2 sm:justify-end">
          {listing.price ? (
            <span className="font-mono-hud text-sm font-bold text-cyan-600 dark:text-cyan-300">GBP {listing.price}</span>
          ) : null}
    ...
    <ListLensShell>
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <HudPanel tone="cyan" className="p-6">
            <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
              <div>
                ...
                <h1 className="mt-5 text-3xl font-black tracking-tight text-[color:var(--brand-text-strong)] sm:text-4xl">
                  ListLens cockpit
                </h1>
                <p className="mt-3 max-w-2xl text-sm leading-6 text-[color:var(--brand-text)]">
          ...
          <HudPanel tone="violet" className="p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-[color:var(--brand-text-strong)]">Guard credits</h2>
                <p className="mt-1 text-sm text-[color:var(--brand-text-muted)]">Point-of-need buyer checks.</p>
              </div>
              <LensOrb icon={WalletCards} tone="violet" />
            </div>
            <p className="mt-5 text-5xl font-black text-[color:var(--brand-text-strong)]">{loading ? "-" : activeData.credits}</p>
            <p className="mt-2 text-sm text-[color:var(--brand-text-muted)]">
        ...
        <section className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
          <HudPanel tone="cyan" className="p-5">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-[color:var(--brand-text-strong)]">Saved Studio drafts</h2>
                <p className="mt-1 text-sm text-[color:var(--brand-text-muted)]">Marketplace-ready drafts and missing-evidence notes.</p>
              </div>
              <Link href="/studio/new" className="text-sm font-semibold text-cyan-300 hover:text-[color:var(--brand-text-strong)]">
                New draft
              </Link>
            </div>
            <div className="space-y-3">
              {activeData.listings.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed border-cyan-300/25 p-8 text-center">
                  <FileText className="mx-auto text-cyan-600 dark:text-cyan-300" size={30} />
                  <p className="mt-3 text-sm font-semibold text-[color:var(--brand-text-strong)]">No drafts yet</p>
                  <p className="mt-1 text-xs text-[color:var(--brand-text-muted)]">Upload photos in Studio to create your first draft.</p>
                </div>
              )}
            </div>
          </HudPanel>
          ...
          <div className="space-y-6">
            <HudPanel tone="violet" className="p-5">
              <div className="mb-5 flex items-center justify-between gap-4">
                <h2 className="text-lg font-bold text-[color:var(--brand-text-strong)]">Recent activity</h2>
                <Link href="/history" className="text-sm font-semibold text-violet-300 hover:text-[color:var(--brand-text-strong)]">
                  History
                </Link>
              </div>
              <div className="space-y-3">
                {activeData.recentActivity.length > 0 ? (
                  ...
                  <p className="rounded-lg border border-dashed border-violet-300/25 p-5 text-sm text-[color:var(--brand-text-muted)]">
            ...
            <HudPanel tone="green" className="p-5">
              <h2 className="text-lg font-bold text-[color:var(--brand-text-strong)]">Rev 1.0 loop</h2>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <HudPanel tone="cyan" className="p-5">
            <h2 className="text-lg font-bold text-[color:var(--brand-text-strong)]">Lens routing</h2>
            <div className="mt-4 space-y-1">
              {liveLens.map((lens) => {
                const Icon = LENS_ICON_MAP[lens.id] ?? Sparkles;
                return (
                  <div key={lens.id} className="flex items-center gap-3 border-b border-[color:var(--brand-outline)] py-3 last:border-0">
                    <LensOrb icon={Icon} tone={lens.accent} size="sm" />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-[color:var(--brand-text-strong)]">{lens.displayName}</p>
                      <p className="text-xs text-[color:var(--brand-text-muted)]">{lens.category}</p>
                    </div>
          ...
          <HudPanel tone="violet" className="p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-[color:var(--brand-text-muted)]">
